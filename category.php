<?php
    ...
    if($_GET){
        ...
        if(isset($_GET["category"])){
            ...
            $query2 = "SELECT * FROM `category` WHERE category='$category'";
...
<body>
    ...
    <div class="container">	
        ...
        <tr class="tr-buku">
				...
				$data = mysqli_query($conn,"SELECT * FROM `book` WHERE category LIKE '%$category%'");
				$jumlah_data = mysqli_num_rows($data);
                
				$total_halaman = ceil($jumlah_data / $batas);
 
				$data_category = mysqli_query($conn,"SELECT * FROM `book` WHERE category LIKE '%$category%' LIMIT $halaman_awal, $batas");
		...
		<nav class="nav-page">
			<ul class="pagination justify-content-center">
				<li class="page-item">
					<a class="page-link" <?php if($halaman > 1){ echo "href='?id=$id&halaman=$previous'"; } ?>>Previous</a>
				</li>
				<?php 
				for($x=1;$x<=$total_halaman;$x++){
				?> 
					<li class="page-item"><a class="page-link" href="?id=<?=$id?>&halaman=<?php echo $x ?>"><?php echo $x; ?></a></li>
				<?php
				}
				?>				
				<li class="page-item">
					<a class="page-link" <?php if($halaman < $total_halaman) { echo "href='?id=$id&halaman=$next'"; } ?>>Next</a>
				</li>
			</ul>
		</nav>
        <?php
