<?php
    ...
    if($_POST){
        if($_POST['isbn']!='' && $_POST['title']!='' && !empty($_FILES['image']['name']) && $_POST['author']!='' && $_POST['category']!='' && $_POST['tag']!='' && $_POST['bookDesc']!='' && $_POST['authDesc']!='' && $_POST['rating']!='' && $_POST['promo']!='' && $_POST['price']!='' && $_POST['page']!='' && $_POST['dimension']!='' && $_POST['weight']!='' && $_POST['publisher']!='' && $_POST['country']!=''){
            $isValid = true;
            ...
            if($isValid){
                ...
                $page = $_POST['page'];
                $dimension = "";
                if($_POST && isset($_POST['dimension'])){
                    $i = count($_POST['dimension']);
                    foreach($_POST['dimension'] as $d){
                        if($i==1){
                            $dimension .= $d." cm";
                        }else{
                            $dimension .= $d." x ";
...
<body>
    ...
    <div class="container mx-auto pt-5">
        <form method="post" enctype="multipart/form-data" class="d-flex flex-column pt-5">
            ...
            <label for="dimension" class="form-label"> Dimension : (ex: 20 x 20 cm) </label>
            <div class="d-inline-block">
                <input type="number" name="dimension[]" min=1 required> x
                <input type="number" name="dimension[]" min=1 required>
            </div>
